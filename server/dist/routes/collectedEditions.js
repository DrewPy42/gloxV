...
router.get('/api/collected-editions', async (req, res) => {
    try {
        const baseQuery = `
      SELECT 
        ce.*,
    ...
    `;
        ...
        const count = await (0, db_1.query)('SELECT COUNT(*) as total FROM collected_edition WHERE deleted_at IS NULL');
...
router.get('/api/collected-editions/:id', async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        // Get collected edition details
        const ceResults = await (0, db_1.query)(`SELECT 
        ce.*,
        p.publisher_name,
        cc.condition_code,
        cc.condition_text,
       ...
       LEFT JOIN condition_code cc ON ce.condition_id = cc.condition_id
       LEFT JOIN location l ON ce.location_id = l.location_id
       WHERE ce.collected_edition_id = ? AND ce.deleted_at IS NULL`, [id]);
       ...
       ORDER BY cei.sequence_order, s.title, i.sort_order`, [id]);
        res.json({
...
router.post('/api/collected-editions', async (req, res) => {
    try {
        const { title, publisher_id, format, isbn, page_count, cover_price, release_date, cover_image_path, condition_id, purchase_price, current_value, value_date, purchase_date, purchase_source, location_id, file_path, backup_path, notes } = req.body;
        if (!title) {
            res.status(400).json({ error: 'title is required' });
            return;
        }
        const result = await (0, db_1.execute)(`INSERT INTO collected_edition (title, publisher_id, format, isbn, page_count,
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        ...
        if (current_value) {
         ...
         VALUES ('collected_edition', ?, ?, NOW())`, [result.insertId, current_value]);
        }
        res.status(201).json({
...
router.put('/api/collected-editions/:id', async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        const { title, publisher_id, format, isbn, page_count, cover_price, release_date, cover_image_path, condition_id, purchase_price, current_value, value_date, purchase_date, purchase_source, location_id, file_path, backup_path, notes, track_value_change } = req.body;
        // Track value change if requested
        if (track_value_change && current_value !== undefined) {
            const currentCe = await (0, db_1.query)('SELECT current_value FROM collected_edition WHERE collected_edition_id = ?', [id]);
            if (currentCe.length > 0 && currentCe[0].current_value !== current_value) {
                await (0, db_1.execute)(`INSERT INTO value_history (entity_type, entity_id, value, recorded_at)
           VALUES ('collected_edition', ?, ?, NOW())`, [id, current_value]);
            }
        }
        const result = await (0, db_1.execute)(`UPDATE collected_edition SET 
...
router.delete('/api/collected-editions/:id', async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        const result = await (0, db_1.execute)('UPDATE collected_edition SET deleted_at = NOW() WHERE collected_edition_id = ? AND deleted_at IS NULL', [id]);
        if (result.affectedRows === 0) {
            res.status(404).json({ error: 'Collected edition not found' });
            return;
        }
...
router.post('/api/collected-editions/:id/issues', async (req, res) => {
    try {
        ...
        const result = await (0, db_1.execute)(`INSERT INTO collected_edition_issue (collected_edition_id, issue_id, 
...
router.delete('/api/collected-editions/:id/issues/:issueId', async (req, res) => {
    try {
        const ceId = parseInt(req.params.id);
        const issueId = parseInt(req.params.issueId);
        const result = await (0, db_1.execute)('DELETE FROM collected_edition_issue WHERE collected_edition_id = ? AND issue_id = ?', [ceId, issueId]);
        if (result.affectedRows === 0) {
...
router.get('/api/collected-editions/:id/value-history', async (req, res) => {
    try {
        const id = parseInt(req.params.id);
...
//# sourceMappingURL=collectedEditions.js.map