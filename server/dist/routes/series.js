...
router.get('/api/series', async (req, res) => {
    ...
    `;
        ...
        if (id) {
            const results = await (0, db_1.query)(`${baseQuery} AND s.series_id = ? ORDER BY COALESCE(s.sort_title, s.title)`, [parseInt(id)]);
            const count = await (0, db_1.query)('SELECT COUNT(*) as total FROM series WHERE deleted_at IS NULL');
...
router.get('/api/series/:id/summary', async (req, res) => {
    try {
    ...
    `;
        const results = await (0, db_1.query)(seriesQuery, [id]);
        if (results.length === 0) {
...
router.post('/api/series', async (req, res) => {
    try {
        const { title, sort_title, issn, publisher_id, comic_age_id, is_limited_series, limited_series_count, notes, default_location_id } = req.body;
        if (!title) {
            res.status(400).json({ error: 'Title is required' });
            return;
        }
        const result = await (0, db_1.execute)(`INSERT INTO series (title, sort_title, issn, publisher_id, comic_age_id, 
...
router.put('/api/series/:id', async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        const { title, sort_title, issn, publisher_id, comic_age_id, is_limited_series, limited_series_count, notes, default_location_id } = req.body;
        const result = await (0, db_1.execute)(`UPDATE series SET 
...
router.delete('/api/series/:id', async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        const result = await (0, db_1.execute)('UPDATE series SET deleted_at = NOW() WHERE series_id = ? AND deleted_at IS NULL', [id]);
        if (result.affectedRows === 0) {
            res.status(404).json({ error: 'Series not found' });
            return;
        }
...
router.get('/api/series/:id/links', async (req, res) => {
    try {
        const id = parseInt(req.params.id);
      ...
      WHERE sl.from_series_id = ? OR sl.to_series_id = ?`, [id, id]);
        res.json({ results });
    }
    catch (err) {
