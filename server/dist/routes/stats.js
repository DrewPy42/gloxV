...
router.get('/api/stats', async (req, res) => {
    try {
        ...
        if (seriesId) {
         ...
         GROUP BY s.series_id`, [parseInt(seriesId)]);
            if (stats.length === 0) {
...
router.get('/api/stats/by-publisher', async (req, res) => {
    try {
...
router.get('/api/stats/by-age', async (req, res) => {
    try {
        const results = await (0, db_1.query)(`SELECT 
        ca.comic_age_id,
        ca.comic_age,
...
router.get('/api/stats/by-condition', async (req, res) => {
    try {
        const results = await (0, db_1.query)(`SELECT 
        cc.condition_id,
        cc.condition_code,
...
router.get('/api/stats/by-location', async (req, res) => {
    try {
        const results = await (0, db_1.query)(`SELECT 
        l.location_id,
        l.storage_type,
...
router.get('/api/stats/high-value', async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || 25;
        // Top valued copies
        const copies = await (0, db_1.query)(`SELECT 
        c.copy_id,
        c.current_value,
        c.format,
        c.grade,
       ...
       LIMIT ${limit}`, []);
        ...
        const collectedEditions = await (0, db_1.query)(`SELECT 
...
router.get('/api/stats/recent', async (req, res) => {
    try {
       ...
       LIMIT ${limit}`, []);
        // Recent series
        const series = await (0, db_1.query)(`SELECT series_id, title, created_at
       FROM series
