...
router.get('/api/tags', async (req, res) => {
    ...
    `;
        ...
        const count = await (0, db_1.query)('SELECT COUNT(*) as total FROM tag');
...
router.get('/api/tags/categories', async (req, res) => {
    try {
        const results = await (0, db_1.query)(`SELECT DISTINCT tag_category, COUNT(*) as tag_count
       FROM tag
...
router.get('/api/tags/:id', async (req, res) => {
    try {
        ...
        const tagResults = await (0, db_1.query)('SELECT * FROM tag WHERE tag_id = ?', [id]);
        if (tagResults.length === 0) {
            res.status(404).json({ error: 'Tag not found' });
            return;
        }
        // Get all entities with this tag
        const usageResults = await (0, db_1.query)(`SELECT 
        tg.entity_type,
        tg.entity_id,
        CASE 
       ...
       FROM taggable tg
       WHERE tg.tag_id = ?
       ORDER BY tg.entity_type, entity_name`, [id]);
        res.json({
            tag: tagResults[0],
            usage: usageResults
        });
...
router.post('/api/tags', async (req, res) => {
    try {
        ...
        const result = await (0, db_1.execute)('INSERT INTO tag (tag_name, tag_category, color) VALUES (?, ?, ?)', [tag_name, tag_category || null, color || null]);
        res.status(201).json({
...
router.put('/api/tags/:id', async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        const { tag_name, tag_category, color } = req.body;
        const result = await (0, db_1.execute)(`UPDATE tag SET 
        tag_name = COALESCE(?, tag_name),
        tag_category = ?,
        color = ?
       WHERE tag_id = ?`, [tag_name, tag_category ?? null, color ?? null, id]);
...
router.delete('/api/tags/:id', async (req, res) => {
    try {
        ...
        const result = await (0, db_1.execute)('DELETE FROM tag WHERE tag_id = ?', [id]);
        if (result.affectedRows === 0) {
            res.status(404).json({ error: 'Tag not found' });
            return;
        }
...
router.post('/api/tags/:id/apply', async (req, res) => {
    try {
        ...
        const result = await (0, db_1.execute)('INSERT INTO taggable (tag_id, entity_type, entity_id) VALUES (?, ?, ?)', [tagId, entity_type, entity_id]);
        res.status(201).json({
...
router.delete('/api/tags/:id/remove', async (req, res) => {
    try {
        const tagId = parseInt(req.params.id);
        const { entity_type, entity_id } = req.body;
        if (!entity_type || !entity_id) {
        ...
        const result = await (0, db_1.execute)('DELETE FROM taggable WHERE tag_id = ? AND entity_type = ? AND entity_id = ?', [tagId, entity_type, entity_id]);
        if (result.affectedRows === 0) {
...
router.get('/api/tags/entity/:type/:id', async (req, res) => {
    try {
        ...
        const results = await (0, db_1.query)(`SELECT t.* 
       FROM tag t
       JOIN taggable tg ON t.tag_id = tg.tag_id
       WHERE tg.entity_type = ? AND tg.entity_id = ?
       ORDER BY t.tag_category, t.tag_name`, [entityType, entityId]);
        res.json({ results });
    }
    catch (err) {
