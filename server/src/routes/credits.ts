...
router.get('/api/credits', async (req: Request, res: Response) => {
  try {
    const baseQuery = `
      SELECT 
        ic.*,
    ...
    const count = await query<RowDataPacket[]>('SELECT COUNT(*) as total FROM issue_credit');
...
router.get('/api/credits/job-titles', async (req: Request, res: Response) => {
  try {
    const results = await query<RowDataPacket[]>(
      `SELECT jt.*, jc.category_name
       FROM job_title jt
       LEFT JOIN job_category jc ON jt.job_category_id = jc.job_category_id
...
router.get('/api/credits/job-categories', async (req: Request, res: Response) => {
  try {
    const results = await query<RowDataPacket[]>(
      `SELECT jc.*, COUNT(jt.job_title_id) as title_count
...
router.post('/api/credits', async (req: Request, res: Response) => {
  try {
    ...
    const result = await execute(
      'INSERT INTO issue_credit (issue_id, person_id, job_title_id, notes) VALUES (?, ?, ?, ?)',
      [issue_id, person_id, job_title_id, notes || null]
    );
...
router.delete('/api/credits/:id', async (req: Request, res: Response) => {
  try {
    const id = parseInt(req.params.id);

    const result = await execute(
      'DELETE FROM issue_credit WHERE issue_credit_id = ?',
      [id]
    );
...
router.post('/api/credits/job-titles', async (req: Request, res: Response) => {
  try {
    ...
    const result = await execute(
      'INSERT INTO job_title (job_title, job_category_id) VALUES (?, ?)',
      [job_title, job_category_id || null]
    );
...
router.post('/api/credits/job-categories', async (req: Request, res: Response) => {
  try {
